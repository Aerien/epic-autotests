...
public class DeliveryCreation extends TestCore {
    private static Delivery delivery;
    private static Catalog catalog;

    @Before
    public void resetPageSettings() {
        catalog = (Catalog) new SiteLogin(getWebDriver())
                .login(test_shop)
                .openMenuItem(AdminBasePage.MenuItem.CATALOG);
    ...
    public void emptyName() {
        delivery.setName("");
        assertEquals(Color.fromString("#B94A48").asRgba(), ((DeliveryTypes) ((Settings)
                catalog.openMenuItem(AdminBasePage.MenuItem.SETTINGS))
                .openSettingsItem(Settings.SettingsItem.DELIVERY_TYPES))
                .addDeliveryMethod(delivery).findElement(DeliveryTypes.delivery_name_input).getCssValue("color"));
    }
